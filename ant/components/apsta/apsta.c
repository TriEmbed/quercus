...
event_handler (void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
  if (event_base == WIFI_EVENT) {
    ...
    case WIFI_EVENT_STA_CONNECTED:
      ESP_LOGI (TAG, "ESP32 Connected to WiFi Network");
      break;
    case WIFI_EVENT_AP_START:
      ESP_LOGI (TAG, "ESP32 soft AP started");
      break;
    case WIFI_EVENT_AP_STACONNECTED:
      ESP_LOGI (TAG, "Station connected to ESP32 soft AP");
      break;
    case WIFI_EVENT_AP_STADISCONNECTED:
      ESP_LOGI (TAG, "Station disconnected from ESP32 soft AP");
      break;
...
wifi_ap (void)
{
  ...
  ESP_ERROR_CHECK (esp_wifi_start ());
  ESP_LOGI (TAG, "WIFI_MODE_AP started. SSID:%s password:%s channel:%d", CONFIG_AP_WIFI_SSID, CONFIG_AP_WIFI_PASSWORD, CONFIG_AP_WIFI_CHANNEL);
  return ESP_OK;
}
#endif
...
wifi_sta (int timeout_ms)
{
  ...
  if (bits) {
    ESP_LOGI (TAG, "WIFI_MODE_STA connected. SSID:%s password:%s", CONFIG_STA_WIFI_SSID, CONFIG_STA_WIFI_PASSWORD);
  } else {
    ESP_LOGI (TAG, "WIFI_MODE_STA can't connected. SSID:%s password:%s", CONFIG_STA_WIFI_SSID, CONFIG_STA_WIFI_PASSWORD);
  }
